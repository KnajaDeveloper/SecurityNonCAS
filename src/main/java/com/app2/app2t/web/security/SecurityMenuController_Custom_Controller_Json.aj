...
privileged aspect SecurityMenuController_Custom_Controller_Json {

  @RequestMapping(value = "/findAppMenu", method = RequestMethod.GET)
  public ResponseEntity<String> SecurityMenuController.findAppMenu() {
      ...
      try {
          ...
          if(list.size() > 0) {
              String appRoleCode = "ADMIN";//list.get(0).get("appRole").toString();
  ...
  public ResponseEntity<String> SecurityMenuController.findEmployeeDetail() {
