...
public class SecurityRestService extends AbstractAPP2Service {
    ...
    public List<Map> getAppRoleService() {
        List<Map> listMap = new ArrayList<>();
        try {
            setWebServicesString("http://" + this.APP2Server + "/security/findNameAppRole");
    ...
    public List<Map> getAppMenuBySecurityService(String appRoleCode) {
				LOGGER.debug("getAppMenuBySecurityService");
        List<Map> listMenu = new ArrayList<>();
        try {
            listMenu = authorizeUtil.getMenuList();
						LOGGER.debug("listMenu.size() {} ",listMenu.size());
            if(listMenu.size() == 0){
                LOGGER.debug("========================> Shooting");
								LOGGER.debug("http://" + this.APP2Server + "/security/findAppMenuByAppRoleCode?appRoleCode=" + appRoleCode);
